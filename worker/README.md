...
接入总控真实订单、保留全部正式等待和识别，仅阻断键鼠指令：

```env
GAME_EXECUTOR_DRY_RUN=true
```

随后按正常方式启动 `game_executor.main`。Worker 会正常接收平台订单、上报中间状态，
每个计划动作输出 `[HID-DRY-RUN]`，但不连接 ESP32。所有等待使用正式时长；演练终态
强制上报 `cancelled / DRY_RUN_NO_HID`，订单不会被标记为游戏交易完成。正式交易前必须
将该配置改回 `false`。

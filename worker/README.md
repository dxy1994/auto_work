...
最终交易确认前的完整游戏截图也由游戏执行端直接上传 RustFS，保存为
`/uploads/trade-screenshots/...` 路径；WebSocket 和订单表只传递、保存该路径。
游戏交易完成后，Monitor 使用同一条聊天指令先发送该截图并关闭聊天页，再打开平台
订单详情页执行商品交付确认，整个链路成功后才将订单标记为已完成。
...
大区所在页码和点击坐标在总控“大区管理”中录入，坐标按 1280×960 游戏客户区填写，

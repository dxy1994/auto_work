<template>
  <div class="page-container">
    ...
    <!-- 编辑弹窗 -->
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑鼠标键盘设备' : '新增鼠标键盘设备'" width="500px" destroy-on-close>
      <el-form :model="form" label-width="80px" ref="formRef" :rules="rules">
        <el-form-item label="设备名称" prop="name">
          <el-input v-model="form.name" placeholder="如：罗技 MK-01" />
        </el-form-item>
        ...
        <el-form-item label="设备信息" prop="device_info">
          <el-input v-model="form.device_info" type="textarea" rows="3" placeholder="设备详细信息..." />
        </el-form-item>
...
import { getMkDevices, createMkDevice, updateMkDevice, deleteMkDevice } from '../api'
...
async function fetchList() {
  loading.value = true
  try {
    const res = await getMkDevices({ page: page.value, page_size: pageSize, keyword: keyword.value })
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await updateMkDevice(editId.value, { ...form })
      ElMessage.success('更新成功')
    } else {
      await createMkDevice({ ...form })

...
public class WebsiteController {
    ...
    public Map<String, Object> list(
        ...
        IPage<Website> result = websiteService.search(category, keyword, PageRequests.of(page, pageSize));
        return Map.of("total", result.getTotal(), "items", result.getRecords());
    }

    @GetMapping("/all")
    public List<Website> listAll() {
        return websiteService.findAllActiveOrdered();
    }

    @GetMapping("/categories")
    public List<String> categories() {
        return websiteService.findDistinctCategories();
    }

    @GetMapping("/{id}")
    public Website get(@PathVariable Integer id) {
    ...
    public Website create(@RequestBody Website payload) {
        payload.setId(null);
        payload.setIsActive(1);
        websiteService.save(payload);
        return payload;
    }

    @PutMapping("/{id}")
    public Website update(@PathVariable Integer id, @RequestBody Website payload) {
        Website w = websiteService.getById(id);
        if (w == null) throw ApiException.notFound("网站不存在");

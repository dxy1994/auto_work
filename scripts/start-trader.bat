...
if not exist "%ENV_FILE%" (
    ...
    copy /y "%WORKER_DIR%\.env.trader.example" "%ENV_FILE%" >nul 2>&1
    if errorlevel 1 (
        echo        [警告] 模板不存在，将使用默认配置
        echo        WORKER_ROLE=trader > "%ENV_FILE%"
...
"%VENV_PYTHON%" -m pip install -r "%WORKER_DIR%\requirements-trader.txt" --quiet
echo        依赖已就绪
...
:: 3. 启动
echo.
echo [3/3] 启动 Trader Worker...
echo.
echo ========================================
echo   角色  : trader（游戏交易执行）
